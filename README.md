...
> **Status:** early public work in progress. The first end-to-end regression is tracked in [Issue #1](../../issues/1). Do not assume the bridge is working until the real ChatGPT acceptance test passes.
...
3. Install/configure `tunnel-client`.
4. Copy the example configuration and fill in local values without committing secrets.
5. Start Blender and its MCP endpoint.
6. Run `./scripts/doctor.sh`.
7. Run `./scripts/acceptance-test.sh` for the locally testable portion.
...
Copy examples instead of editing tracked files with secrets:
...
`.env` and `config/tunnel-client.local.yaml` are ignored by Git.
...
See [`docs/troubleshooting.md`](docs/troubleshooting.md). The current 502 regression is tracked in [Issue #1](../../issues/1), and its eventual root cause will be turned into a doctor check and regression note where practical.
